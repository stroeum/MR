...
rhot             = rhos + rho;
Et               = (Er.^2 + Ez.^2).^(1/2);
% SYM.Et           = duplicate(1/2*log18(Et*(1e-5)^2));              %log(_kV/cm)
SYM.Et           = duplicate(Et*1e-5);              %log(_kV/cm)
SYM.rhot         = duplicate(rhot*1e9);                            %_nC/m^3
...
XTick = get(gca,'XTick');
set(gca,'XMinorTick','on','YMinorTick','on','XTickLabel',abs(XTick)');
title('\rho_t=\rho_s+\rho_f (nC/m^3)','fontsize',18);
set(gca,'FontSize',18);

subplot(222);
imagescUnsgnLog(SYM.r,SYM.z,SYM.Et,-6,0);
axis([-r(end)*1e-3 r(end)*1e-3 z(1)*1e-3 z(end)*1e-3])
xlabel('r (km)','fontsize',18)
ylabel('z (km)','fontsize',18)
XTick = get(gca,'XTick');
set(gca,'XMinorTick','on','YMinorTick','on','XTickLabel',abs(XTick)');
set(gca,'FontSize',18)
title('|E_t|=(E_r^2+E_z^2)^{1/2} (kV/cm)','fontsize',18);
...
legend('\rho_s','\rho','\rho_t','location','SouthEast');
legend('boxoff')
set(gca,'FontSize',18);
...
%%
hgexport(gcf,'~/Desktop/GJ/dynamics.eps');