/** @file matrix.h this file presents a base class for matrixes
 * and the operations available on it. */
...
protected:
	...
	virtual bool init();							// Initialize the dimension of the matrix and its
													// content (all values are set to 0)
													// according to the value of <i>_iNbItem</i>.
													// @return true if the memory could be allocated,
													// false in the other case.
	CMatrix& operator=(const CMatrix& matrix);		// Overloading affectation operator
													// @throw ENotEnoughMemory if there is not enough
													// memory to complete the copy.	
	CMatrix& operator+=(const CMatrix& matrix);		// Overloading self addition operator. 
													// throw EAdditionImpossible is the addition is
...
public:
	double* pElems;									// An array of <i>iNbElem</i> doubles which are the
													// content of the matrix. 
													// impossible.	
	class EMatrix : public exception{};				// Base class for exceptions raised by methods of
													// the CMatrix class.
	class ENotEnoughMemory : public EMatrix{};		// Exception raised if memory is missing to complete
													// an operation.
	class EAdditionImpossible : public EMatrix{};	// Exception raised if the addition of 2 matrix is
													// impossible.
			
	CMatrix(int iNbElem=0);							// Default constructor. 
													// @throw ENotEnoughMemory if there is not enough
													// memory to complete the creation.
...
protected:
	...
	double ** pMatrix2d;
	virtual void freeMemIfRequired(void);			// Free the memory occupied by _pMatrix3d if
													// necessary.
	virtual bool init();							// Initialize the dimension of the matrix and its
													// content (all values are set to 0).
													// @return true if the memory could be allocated,
...
public:
	double *** pMatrix3d;							// A shortcul to access the elements stored in the
													...
													//		* The element [x, y, z] is available at the
													//		  adress : _pMatrix3d[x][y][z]
	...
	inline double** operator[](int x) __attribute__((always_inline))
	{return pMatrix3d[x];};
													// Overloading the bracket operator so that an
													// element of the matrix can be accessed using
													// [x][y][z]
	inline double** operator[](int x) const __attribute__((always_inline))
	{return pMatrix3d[x];};
													// Overloading the bracket operator so that an
													// element of the matrix can be accessed using
													// [x][y][z]
	inline double& operator()(int x, int y, int z) __attribute__((always_inline))
	{return pMatrix3d[x][y][z];};
													// Overloading the parenthesis operator so that an
													// element of the matrix can be accessed using (x,
													// y, z). 
	inline const double& operator()(int x, int y, int z) const __attribute__((always_inline))
	{return pMatrix3d[x][y][z];};	
													// Overloading the parenthesis operator so that an
													// element of the matrix can be accessed using (x,
													// y, z).
	...
	CMatrix3D operator+(const CMatrix3D& matrix) const; 
													...
													//	* @throw EAdditionImpossible is the addition
													//	  is impossible.
													//  * @throw ENotEnoughMemory if there is not
													//	  enough memory to complete the creation.	
	CMatrix3D operator-(const CMatrix3D& matrix) const; 
	...
	void fwrite(string);							// Write Matrix in a file	
}; // class CMatrix3D
/*
std::ostream& operator<<(std::ostream& os, const CMatrix3D& matrix); 
													// Overloading display operator.
*/
/**************************************************************************************/
...
protected:
	int xSize;										// Width of the matrix.
	int ySize;										// Height of the matrix.
	double ** pMatrix2d;							// A shortcul to access the elements stored in the
													// <i>pElems</i> variable
													// * inherited from the base class.
													// * The element [x, y] is available at the adress :
													//   pMatrix2d[x][y]
	virtual void freeMemIfRequired(void);			// Free the memory occupied by _pMatrix2d if
													// necessary.
	virtual bool init();							// Initialize the dimension of the matrix and its
													// content (all values are set to 0). 
													// @return true if the memory could be allocated,
...
public:
	...
	double* operator[](int x) __attribute__((always_inline))
	{return pMatrix2d[x];};	
													// Overloading the bracket operator so that an
													// element of the matrix can be accessed
													// using [x][y]
	
	double* operator[](int x) const __attribute__((always_inline))
	{return pMatrix2d[x];};
													// Overloading the parenthesis operator so that an
													// element of the matrix can be accessed
													// using (x, y).
	
	double& operator()(int x, int y) __attribute__((always_inline))
	{return pMatrix2d[x][y];};
													// Overloading the parenthesis operator so that an
													// element of the matrix can be accessed
													// using (x, y).
	
	const double& operator()(int x, int y) const __attribute__((always_inline))
	{return pMatrix2d[x][y];};
													// Overloading the parenthesis operator so that an
													// element of the matrix can be accessed
													// using (x, y).
...
/*
std::ostream& operator<<(std::ostream& os, const CMatrix2D& matrix);
													// Overloading display operator.
 */
/**************************************************************************************/
...
public:
	...
	{return pElems[x];};
													...
													// element of the matrix can be accessed
													// using (x).
	const double& operator()(int x) const __attribute__((always_inline))
	{return pElems[x];};
													// Overloading the parenthesis operator so that an
													// element of the matrix can be accessed
													// using (x).
